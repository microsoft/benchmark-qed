...
The AutoQ component generates synthetic query classes based on the scope and source of the dataset. 
...
These two dimensions produce four major query classes. In addition, AutoQ supports *data-linked* queries—a variant of data-driven local queries that combine multiple local questions sharing named entities into multi-hop questions (bridge, comparison, intersection, temporal).

AutoQ can be configured to generate any number and distribution of synthetic queries along these classes.

In addition to queries, AutoQ can generate **assertions**—testable factual statements that serve as "unit tests" for evaluating answer accuracy. Assertions are automatically generated for data-driven queries, including data-linked queries (not available for activity-driven queries), and can optionally be validated against source data to ensure quality.

> **Note:** AutoQ generates queries and assertions only; it does **not** produce reference (ground truth) answers. Since assertions are generated (and optionally validated) by LLMs, they may contain inaccuracies and should be manually reviewed before use.

> **Tip: Adding Assertions to Existing Questions**
>
> If you have existing data-local, data-linked, or data-global questions that were generated without assertions, you can generate assertions for them retrospectively. Please refer to the [Assertion Generation Notebook](notebooks/assertion_gen.ipynb) for step-by-step instructions.
...
3. **Assertion-based scoring**: AutoE can evaluate RAG answers against assertions (either user-defined or automatically generated by AutoQ). The LLM judge determines whether each answer satisfies the given assertions, providing binary pass/fail scores. This method is particularly useful for testing specific factual requirements or compliance with predefined criteria. When using AutoQ-generated assertions, this enables automated end-to-end evaluation of answer accuracy for data-local, data-linked, and data-global questions.

In addition to the core methods above, AutoE provides several specialized subcommands:

- **`differential-pairwise-scores`**: A variant of pairwise comparison that uses an extract-and-judge approach to reduce length and formatting bias, by extracting the common and unique content of each answer and judging only the unique content. Its output is compatible with the standard pairwise significance pipeline.
- **`hierarchical-assertion-scores`**: Scores hierarchical assertions (assertions with supporting sub-assertions), with support for comparing multiple RAG methods and significance testing.
- **`chunk-assertion-scores`**: Evaluates retrieved chunks directly (rather than synthesized answers) against per-question assertions, reporting coverage metrics at different `k` values. Useful for assessing retrieval quality independent of answer generation.

For the full list of AutoE subcommands and their configuration, see the [AutoE CLI Documentation](cli/autoe.md).
