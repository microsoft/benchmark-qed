...
class ActivityGlobalQuestionGen(BaseQuestionGen):
    ...
    async def agenerate(
        ...
        log.info(
            f"Generated {len(results)} candidate questions for {len(self.activity_context.task_contexts)} tasks"
        )
    ...
    async def _agenerate_single_task(
        ...
        """Generates questions based on dataset description, persona and task descriptions."""
        try:
            ...
            if _j == {}:
                log.error(
                    f"Failed to parse questions response: {model_response.output.content}"
                )
                return []
