...
class ActivityLocalQuestionGen(BaseQuestionGen):
    ...
    async def agenerate(
        ...
        log.info(f"Generated {len(results)} candidate questions")
        ...
        log.info(f"Number of questions per entity: {num_questions_per_entity}")
        log.info(f"Number of entities: {len(entity_questions)}")
        ...
        log.info(
            f"Retained {len(filtered_results)} questions post-filtering by entity distribution"
        )
        missing_questions = num_questions - len(filtered_results)
        if missing_questions > 0:
            log.info(f"Filling in {missing_questions} missing questions")
            remaining_questions = [
                question
                for question in results
                if question.text not in unique_question_texts
    ...
    async def _agenerate_single_task(
        ...
        """Generates questions for a single task context using a chain of thought process"""
        try:
            if len(task_context.entities) == 0:
                log.warning(f"No entities found for task: {task_context.task}")
                return []
            ...
            questions, _j = try_parse_json_object(model_response.output.content)
            if _j == {}:
                log.error(f"Error parsing JSON response: {questions}")
                return []
            parsed_questions = json.loads(questions).get("questions", [])
            log.debug(f"Parsed questions: {parsed_questions}")
