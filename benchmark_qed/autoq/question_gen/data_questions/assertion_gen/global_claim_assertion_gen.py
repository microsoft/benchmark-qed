...
from benchmark_qed.autod.data_processor.text_utils import try_parse_json_object, num_tokens
...
class GlobalClaimAssertionGenerator(BaseAssertionGenerator):
    ...
    def __init__(
        ...
        self.map_prompt = map_system_prompt if map_system_prompt else load_template_file(
            ASSERTION_GEN_PROMPTS_PATH / "assertions" / "global_claim_assertion_map_prompt.txt"
        )
        if isinstance(self.map_prompt, str):
            self.map_prompt = Template(self.map_prompt)

        self.reduce_prompt = reduce_system_prompt if reduce_system_prompt else load_template_file(
            ASSERTION_GEN_PROMPTS_PATH / "assertions" / "global_claim_assertion_reduce_prompt.txt"
        )
    ...
    async def agenerate_assertions(
        ...
        final_assertions = await self.generate_reduce_response(question_text, reduce_context)
    ...
    def build_map_context(self, claims: list[ClaimDict]) -> list[list[ClaimDict]]:
        ...
        batch_size = self.batch_size if self.batch_size and self.batch_size > 0 else len(claims)
        ...
        log.info(f"MAP CONTEXT: Created {len(batches)} batches from {len(claims)} simple claims")
        return batches
    ...
    async def generate_map_responses(
        ...
        log.info(f"MAP RESPONSES: Processing {len(claim_batches)} batches in parallel")
        
        async def process_batch_with_semaphore(batch: list[ClaimDict]) -> list[Assertion]:
            async with self._semaphore:
                # Use the shared local generator for map phase processing
                result = await self.local_generator.agenerate_assertions(question_text, claims=batch)
        ...
        for i, result in enumerate(batch_results):
            if isinstance(result, Exception):
                log.error(f"Error processing batch {i} in MAP step: {result}")
                continue
            if isinstance(result, list):
                map_responses.append(result)
        
        log.info(f"MAP RESPONSES: Successfully processed {len(map_responses)} out of {len(claim_batches)} batches")
    ...
    def build_reduce_context(
        ...
        log.info(f"REDUCE CONTEXT: Merging {len(initial_assertions)} assertions from {len(map_responses)} batches")
        ...
        log.info(f"REDUCE CONTEXT: Ranked {len(ranked_assertions)} unique assertions by score and source count")
        ...
        for i, assertion in enumerate(ranked_assertions, 1):
            ...
            assertion_text = f"ID: assertion_{i}\nStatement: {statement}\nScore: {score}"
            ...
            if current_tokens + assertion_tokens > self.max_data_tokens:
                log.info(f"REDUCE CONTEXT: Reached token limit at assertion {i-1}, stopping at {current_tokens} tokens")
                break
        ...
        log.info(f"REDUCE CONTEXT: Selected {len(selected_assertions)} of {len(ranked_assertions)} assertions within {current_tokens} tokens (limit: {self.max_data_tokens})")
        return selected_assertions, formatted_text

    def _build_assertion_mapping(self, assertions: list[Assertion]) -> dict[str, dict[str, Any]]:
    ...
    def _map_sources_and_aggregate(
        ...
        Returns:
            Tuple of (mapped_sources, aggregated_source_chunks)
        """
        mapped_sources = []
        ...
        if assertion_mapping and sources:
            ...
            # If no mapping available, keep original sources
            mapped_sources = sources
            
        return mapped_sources, aggregated_source_chunks
    ...
    def _validate_consolidated_assertions(
        ...
        for assertion in consolidated_assertions:
            ...
            if statement:
                ...
                mapped_sources, aggregated_source_chunks = self._map_sources_and_aggregate(
                    sources, assertion_mapping
                )
    ...
    async def generate_reduce_response(
        ...
        if j == {}:
            log.warning(f"Failed to parse consolidation response, returning original: {response}")
            return self._rank_and_limit_assertions(unique_assertions, self.max_assertions)
        ...
        log.info(f"Successfully consolidated {len(unique_assertions)} assertions into {len(validated_assertions)}")
        return validated_assertions
