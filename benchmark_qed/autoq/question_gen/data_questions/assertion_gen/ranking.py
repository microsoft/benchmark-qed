...
def calculate_rrf_scores(
    ...
    Example:
        ...
        ...     },
        ... ]
        >>> tied_scores = calculate_rrf_scores(
        ...     tied_docs,
        ...     lambda d: d[
        ...         "score"
        ...     ],
    ...
    source_count_ranks = calculate_dense_ranks(
        items, source_count_key_func, reverse=True
    )
    ...
    for item in items:
        ...
        rrf_score = (score_weight * (1 / (k + score_rank))) + (
            source_count_weight * (1 / (k + source_rank))
        )
...
def calculate_dense_ranks(
    items: list[Any], key_func: Callable[[Any], float | int], reverse: bool = True
) -> dict[int, int]:
    """
