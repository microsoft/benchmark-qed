...
class DataGlobalQuestionGen(BaseQuestionGen):
    ...
    async def agenerate(
        ...
        for i in range(0, len(question_contexts), self.concurrent_coroutines):
            log.info(
                f"Processing categories {i} to {min(i + self.concurrent_coroutines, len(question_contexts))} of {len(question_contexts)} categories..."
            )
        ...
        log.info(
            f"Generated {len(results)} candidate questions from {len(self.local_questions)} local questions."
        )
    ...
    def _generate_question_context(
        ...
        sorted_cats = sorted(
            [
                c
                for c in category_to_question.keys()
                if len(category_to_question[c]) > 1
            ],
        ...
        log.info(f"Number of initial categories: {len(category_to_question)}")
        log.info(
    ...
    async def _agenerate_single_chain(
        ...
        """
        Generates questions for a single input text
        """
        try:
            async with self.semaphore:
                ...
                questions, _j = try_parse_json_object(questions_result.output.content)
                if _j == {}:
                ...
                for question in parsed_questions["questions"]:
                    if question in question_set:
                        log.info(f"Duplicate question to be filtered out: {question}")
                        continue
