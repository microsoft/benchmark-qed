...
def plot_assertion_accuracy_by_rag_method(
    ...
    if sort_by_mean:
        ...
        pivot_summary_reset["mean_accuracy"] = pivot_summary_reset.select_dtypes(
            include="number"
        ).mean(axis=1)
        pivot_summary_reset = pivot_summary_reset.sort_values(
            "mean_accuracy", ascending=True
        )
    ...
    question_set_columns = [
        col
        for col in pivot_summary_reset.columns
